...
export interface UseCloneToolOptions<T extends { id: string } = { id: string }, TPose = unknown> {
   ...
   *  AUTO_POSE_DESCRIPTOR Canvas uses for its own hitBody fallback. */
...
export function useCloneTool<T extends { id: string }, TPose = unknown>(
  ...
  return useMemo(() => {
    ...
    const defaultHitBody = (worldX: number, worldY: number): string | null => {
    ...
    return defineTool<CloneScratch>({
      ...
      pointer: {
        onDown: (_e, ctx) => {
          ...
          const hit = optsRef.current.hitBody ?? defaultHitBody;
          const id = hit(ctx.worldX, ctx.worldY);
