...
function setup(overrides: Partial<Parameters<typeof useZoom>[0]> = {}) {
  const setZoom = vi.fn();
  const setPan = vi.fn();
  const opts = {
    zoom: 1,
  ...
  const { result } = renderHook(() => useZoom(opts));
  return { result, setZoom, setPan, opts };
}

describe('useZoom — clamp policy', () => {
...
describe('useZoom — focal-point invariant', () => {
  ...
  it('zoomTo without focal uses viewport center', () => {
    ...
    const pan = { x: 0, y: 0 };
    const { result } = renderHook(() =>
      useZoom({ zoom, setZoom, pan, setPan, viewport }),
    );
