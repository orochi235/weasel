/**
 * Smoke test for `scripts/gen-font.ts`. Lives under `src/` (rather than
 * next to the script) because `scripts/` isn't covered by any vitest
 * project — the `kit` project's include glob is `src/**\/*.test.{ts,tsx}`
