...
describe('useSelection', () => {
  ...
  it('adapterMethods.getSelection returns current', () => {
    ...
    act(() => result.current.adapterMethods.setSelection(['x', 'y']));
    expect(result.current.adapterMethods.getSelection()).toEqual(['x', 'y']);
    expect(result.current.current).toEqual(['x', 'y']);
  });
