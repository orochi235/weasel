...
describe('defaultDuplicateAction', () => {
  ...
  it('id="duplicate", label="Duplicate", binding={key:"d", mod:true}', () => {
    const a = defaultDuplicateAction({ getSelection: () => [asNodeId('a')], cloneNode, applyBatch: vi.fn() });
  ...
  it('uses default offset {dx:8, dy:8} passed to cloneNode', () => {
    ...
    const a = defaultDuplicateAction({ getSelection: () => [asNodeId('a')], cloneNode: clone, applyBatch: vi.fn() });
    a.run();
