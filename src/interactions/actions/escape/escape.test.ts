...
describe('useEscape', () => {
  ...
  it('clearSelection identity is stable across renders', () => {
    ...
    const { result, rerender } = renderHook(() => useEscape(helpers.adapter));
  ...
  describe('keyboard', () => {
    it('Escape on document fires by default', () => {
      const helpers = makeAdapter(['a']);
      renderHook(() => useEscape(helpers.adapter));
      act(() => {
    ...
    it('enableKeyboard: false disables binding', () => {
      const helpers = makeAdapter(['a']);
      renderHook(() => useEscape(helpers.adapter, { enableKeyboard: false }));
      act(() => {
        document.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true, cancelable: true }));
      });
      expect(helpers.batches).toEqual([]);
    ...
    it('listener is removed on unmount', () => {
      const helpers = makeAdapter(['a']);
      const { unmount } = renderHook(() => useEscape(helpers.adapter));
      unmount();
      act(() => {
        document.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true, cancelable: true }));
      });
