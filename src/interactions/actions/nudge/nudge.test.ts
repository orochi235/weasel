...
describe('useNudge', () => {
  ...
  it('custom step + shiftStep flow through', () => {
    const helpers = makeAdapter(['a']);
    const { result } = renderHook(() => useNudge(helpers.adapter, { translatePose, step: 4, shiftStep: 40 }));
    act(() => { result.current.nudge('right'); });
  ...
  it('nudge identity stable across renders', () => {
    const helpers = makeAdapter(['a']);
    const { result, rerender } = renderHook(() => useNudge(helpers.adapter, { translatePose }));
  ...
  describe('keyboard', () => {
    it('ArrowRight fires nudge right by default', () => {
      const helpers = makeAdapter(['a']);
      renderHook(() => useNudge(helpers.adapter, { translatePose }));
      act(() => {
    ...
    it('enableKeyboard: false disables binding', () => {
      const helpers = makeAdapter(['a']);
      renderHook(() => useNudge(helpers.adapter, { translatePose, enableKeyboard: false }));
      act(() => {
        document.dispatchEvent(new KeyboardEvent('keydown', { key: 'ArrowRight', bubbles: true, cancelable: true }));
      });
      expect(helpers.batches).toEqual([]);
    ...
    it('listener removed on unmount', () => {
      const helpers = makeAdapter(['a']);
      const { unmount } = renderHook(() => useNudge(helpers.adapter, { translatePose }));
      unmount();
      act(() => {
        document.dispatchEvent(new KeyboardEvent('keydown', { key: 'ArrowRight', bubbles: true, cancelable: true }));
      });
