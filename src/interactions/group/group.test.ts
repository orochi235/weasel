...
import { useGroupAction, useUngroupAction, type GroupActionAdapter } from './group';
...
describe('useGroupAction', () => {
  it('wraps the current selection in a new virtual group and selects it', () => {
    ...
    const { result } = renderHook(() =>
      useGroupAction(h.adapter, { newGroupId: () => 'g1' }),
    );
  ...
  it('is a no-op when selection is below minMembers (default 2)', () => {
    ...
    const { result } = renderHook(() => useGroupAction(h.adapter));
  ...
  it('uses a custom label when provided', () => {
    const h = makeHarness(['a', 'b']);
    const { result } = renderHook(() =>
      useGroupAction(h.adapter, { newGroupId: () => 'g1', label: 'Bundle' }),
    );
