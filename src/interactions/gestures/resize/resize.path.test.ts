...
import { pathPoseGeometry } from '../../../paths/poseGeometry';
import { polygonFromPoints, type PolygonPath, type Path } from '../../../paths';
...
describe('useResizeInteraction — non-rect TPose via pathPoseGeometry', () => {
  it('drives a polygon pose: emitted op carries a remapped polygon scaled against drag bounds', () => {
    ...
    const { result } = renderHook(() =>
      useResizeInteraction<{ id: string }, Path>(adapter, {
  ...
  it('overlay.currentPose remains a Path (lerped) and overlay.targetPose is the final remapped Path', () => {
    ...
    const { result } = renderHook(() =>
      useResizeInteraction<{ id: string }, Path>(adapter, { geometry: pathPoseGeometry }),
    );
