...
import { useClipboardAction } from './clipboardAction';
import type { ClipboardActionAdapter } from './clipboardAction';
import type { Op } from '../..';
...
function makeAdapter(initial: { selection?: string[]; offset?: { dx: number; dy: number } } = {}) {
  ...
  const adapter: ClipboardActionAdapter<Obj> = {
    commitInsert: () => null,
...
describe('useClipboardAction', () => {
  it('exposes copy / cut / paste / isEmpty; isEmpty starts true', () => {
    const { adapter } = makeAdapter();
    const { result } = renderHook(() =>
      useClipboardAction(adapter, { getSelection: () => [] }),
    );
  ...
  it('copy snapshots selection without emitting a batch', () => {
    ...
    const { result } = renderHook(() =>
      useClipboardAction(helpers.adapter, { getSelection: () => ['a'] }),
    );
  ...
  it('paste flows through to InsertOps + SetSelection like useClipboard', () => {
    const helpers = makeAdapter();
    helpers.seed({ id: 'a', x: 0, y: 0 });
    const { result } = renderHook(() =>
      useClipboardAction(helpers.adapter, { getSelection: () => ['a'] }),
  ...
  describe('cut', () => {
    ...
    it('empty selection: no batch, returns []', () => {
      const helpers = makeAdapter();
      const { result } = renderHook(() =>
        useClipboardAction(helpers.adapter, { getSelection: () => [] }),
      );
    ...
    it('custom cutLabel flows through', () => {
      ...
      helpers.seed({ id: 'a', x: 0, y: 0 });
      const { result } = renderHook(() =>
        useClipboardAction(helpers.adapter, {
          getSelection: () => ['a'],
          cutLabel: 'Cut node',
        }),
      );
    ...
    it('falls back to stub object when getObject is omitted', () => {
      ...
      const { result } = renderHook(() =>
        useClipboardAction(adapter, { getSelection: () => ['a'] }),
      );
      act(() => { result.current.cut(); });
      expect(helpers.batches).toHaveLength(1);
