...
export const flipAction: Action & { requires: string[] } = {
  ...
  requires: ['selection', 'scene', 'applyOps', 'geometryProjection', 'poseComposition'],
  invoker: {
    timing: 'immediate',
    run: (deps, params) => {
      ...
      flipSelection(selection, scene, axis, pivot, deps.poseComposition, applyOps, geometryProjection);
    },
  },
