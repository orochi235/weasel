...
export interface ArrayAdapterConfig<TNode extends { id: string }, TPose> {
  ...
  /** How to read and rewrite item poses. Default `RECT_POSE_DESCRIPTOR`,
   *  which reads the pose as its own AABB — the canonical axis-aligned rect
...
export function arrayAdapter<TNode extends { id: string }, TPose>(
  ...
  const adapter: ArrayAdapter<TNode, TPose> = {
    ...
    commitPaste: (
      ...
      // the cluster's union AABB at the drop point — matches the
      // `ClipboardDemo` reference wiring.
      let dx = offset.dx;
      let dy = offset.dy;
      if (ctx?.dropPoint) {
        ...
        // Defensive: if the descriptor returned nothing usable, fall back to offset.
