...
function mount(
  scene: Scene<Data, 'main', RectPose>,
...
describe('<SceneCanvas> installs the scene wiring', () => {
  ...
  it('paints the derived path in a preview ghost', () => {
    const { scene, edge } = makeEdgeScene();
    // A tool is the preview-ghost layer's source of in-flight ids and poses.
    const ghostTool = {
      id: 'ghost-source',
      previewIds: () => [edge as string],
      previewPose: (id: string) => (id === (edge as string) ? pose(0) : null),
    } as unknown as AnyTool;
    const ghost = mount(scene, { 'ghost-source': ghostTool }).previewGhost.layer;
    expect(pathCommands(ghost.draw(null, VIEW, DIMS))).toEqual([EXPECTED]);
  });
});
