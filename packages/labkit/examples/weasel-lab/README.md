...
Proof-of-concept: labkit's Lab/Trial chrome wrapping a weasel scene-graph canvas.
...
- A labkit `<Lab>` shell (toolbar with Save/Clone/Reset/Close, sidebar with control panel, status bar, multi-trial tabs) wrapping a `<SceneCanvas>` from `@weasel-js/core`.
...
- **Reset / Clone:** work via the persistence bridge above. Reset clears `state.scene` → instrument rebuilds with `INITIAL_NODES`. Clone produces a new trial whose initial state is a deep copy of the source — its scene hydrates from that copy on first mount.
