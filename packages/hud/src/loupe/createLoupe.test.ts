...
describe('createLoupe', () => {
  ...
  it('a lens that is a view reads its canvas\u2019s own pane without a region', () => {
    const hud = createHud();
    const frame = bindHost(hud);
    const el = document.createElement('canvas');
    el.width = 820; el.height = 400;
    ...
    const pane = { x: 420, y: 20, width: 380, height: 360 };
    const views = {
      addView: vi.fn(() => ({ id: 'lens', draw: () => [], remove: () => {} })),
      getSurfaceRect: () => pane,
    };
    const loupe = createLoupe({ hud, canvas: el, requestRedraw: () => {}, views });
    loupe.aimAt({ x: 300, y: 50 });
    frame();
    reads.length = 0;
    loupe.pick();
    expect(reads).toEqual([{ x: 720, y: 400 - 70 - 1 }]);
    loupe.dispose();
  });
