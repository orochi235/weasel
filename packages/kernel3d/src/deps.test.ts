...
import { dot, len, sub, type Vec3 } from '@weasel-js/geom/3d';
...
function viewport(overrides: Partial<Viewport3d> = {}): Viewport3d {
  return {
    camera: createCamera({ distance: 12, pitch: 0.4, target: { x: 0, y: 0.5, z: 0 } }),
    width: WIDTH,
    height: HEIGHT,
    ...overrides,
  };
...
function depthOf(vp: Viewport3d, p: Vec3): number {
...
describe('createNodeAtPoint', () => {
  ...
  it('follows the camera: orbiting changes what is under the cursor', () => {
    ...
    vp = { ...vp, camera: createCamera({ distance: 12, pitch: 1.5, target: { x: 0, y: 0.5, z: 0 } }) };
  ...
  it('picks nothing when the camera casts no ray', () => {
    ...
    const flat = viewport({ camera: createCamera({ distance: 0, target: { x: 0, y: 0.5, z: 0 } }) });
...
describe('createInsert', () => {
  it('stands a new box on the ground plane under the drag rectangle', () => {
    ...
    expect(pose.position.y).toBeCloseTo(pose.scale.y / 2, 6);
  });
  ...
  it('returns null when the rectangle points at the sky', () => {
    ...
    const shallow = viewport({
      camera: createCamera({ distance: 12, pitch: 0.05, target: { x: 0, y: 0.5, z: 0 } }),
    });
...
describe('createPoseDescriptor', () => {
  ...
  it('draws a nearer solid bigger', () => {
    const vp = viewport();
    const descriptor = createPoseDescriptor(world(scene, () => vp));
    const near = descriptor.getBounds(pose3({ x: 0, y: 0.5, z: 4 }));
    const far = descriptor.getBounds(pose3({ x: 0, y: 0.5, z: -4 }));
  ...
  describe('remapBounds — the screen map at the pose’s own depth', () => {
    ...
    it('leaves a pose alone when the rectangle does not move', () => {
      const pose = pose3({ x: 0, y: 0.5, z: 0 });
      const same = descriptorAt().remapBounds(pose, boxOf(pose), boxOf(pose));
      expect(same.position.x).toBeCloseTo(pose.position.x, 6);
      expect(same.position.y).toBeCloseTo(pose.position.y, 6);
      expect(same.position.z).toBeCloseTo(pose.position.z, 6);
      expect(same.scale.x).toBeCloseTo(pose.scale.x, 6);
    });

    it('scales uniformly by the rectangle it is given', () => {
      const pose = pose3({ x: 0, y: 0.5, z: 0 }, { x: 1, y: 2, z: 3 });
      const src = boxOf(pose);
      const bigger = descriptorAt().remapBounds(pose, src, grow(src, 2));
      expect(bigger.scale.x).toBeCloseTo(2, 6);
      expect(bigger.scale.y).toBeCloseTo(4, 6);
      expect(bigger.scale.z).toBeCloseTo(6, 6);
    });
    ...
    it('keeps the depth it was handed', () => {
      ...
      expect(moved.position.x).not.toBeCloseTo(pose.position.x, 3);
  ...
  describe('fromBounds — the box the rectangle covers at that depth', () => {
    ...
    it('lands where the rectangle points', () => {
      const d = descriptorAt();
      const template = pose3({ x: 0, y: 0.5, z: 0 });
    ...
    it('carries none of the template but its depth', () => {
      const d = descriptorAt();
      const template = pose3({ x: 0, y: 0.5, z: 0 }, { x: 4, y: 0.25, z: 4 });
    ...
    it('gives a bigger rectangle a bigger box', () => {
      ...
      expect(large.scale.y).toBeGreaterThan(small.scale.y);
      expect(large.scale.z).toBeGreaterThan(small.scale.z);
    });
    ...
    it('gives the unnamed third extent the mean of the two it has', () => {
      ...
      const overhead = createPoseDescriptor(world(scene, () => viewport({
        camera: createCamera({ distance: 12, pitch: 0, yaw: 0, target: { x: 0, y: 0.5, z: 0 } }),
      })));
      const built = overhead.fromBounds({ x: 300, y: 240, width: 200, height: 100 }, pose3({ x: 0, y: 0.5, z: 0 }));
      expect(built.scale.z).toBeCloseTo(Math.sqrt(built.scale.x * built.scale.y), 3);
  ...
  it('turns a screen delta into world movement across the ground plane', () => {
    const vp = viewport();
    const descriptor = createPoseDescriptor(world(scene, () => vp));
    const start = pose3({ x: 0, y: 0.5, z: 0 });

    const moved = descriptor.translate!(start, 120, 0);
    expect(moved.position.x).toBeGreaterThan(start.position.x);
    expect(moved.position.y).toBeCloseTo(start.position.y, 6);
  });

  it('keeps a dragged solid at its own height', () => {
    const vp = viewport();
    const descriptor = createPoseDescriptor(world(scene, () => vp));
    const start = pose3({ x: 0, y: 3, z: 0 });
    const moved = descriptor.translate!(start, 40, 40);
    expect(moved.position.y).toBeCloseTo(3, 6);
  });

  it('moves further for the same drag when the camera is further away', () => {
    const near = createPoseDescriptor(world(scene, () => viewport({
      camera: createCamera({ distance: 6, pitch: 0.4, target: { x: 0, y: 0.5, z: 0 } }),
    })));
    const far = createPoseDescriptor(world(scene, () => viewport({
      camera: createCamera({ distance: 30, pitch: 0.4, target: { x: 0, y: 0.5, z: 0 } }),
    })));
    const start = pose3({ x: 0, y: 0.5, z: 0 });
    const nearMoved = near.translate!(start, 100, 0).position.x;
    const farMoved = far.translate!(start, 100, 0).position.x;
  ...
  it('keeps dragging through the last camera that cast a ray', () => {
    ...
    vp = viewport({ camera: createCamera({ distance: 0, target: { x: 0, y: 0.5, z: 0 } }) });
  ...
  it('says it cannot rotate', () => {
    ...
    expect(descriptor.supportsRotation!(pose3({ x: 0, y: 0, z: 0 }))).toBe(false);
  });
  ...
  it('bounds a sphere as a sphere once it can see the node', () => {
    ...
    const id = scene.add({
      kind: 'leaf',
      layer: 'solids',
      pose: pose3({ x: 0, y: 0.5, z: 0 }, { x: 2, y: 0.5, z: 0.5 }),
      data: { round: true },
    });
