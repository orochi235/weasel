...
**The categorical swatch tokens in `packages/theme` are landed but their shape is
not settled.** `swatch-*` currently holds a ten-color, no-yellow set (commit
"replace the categorical swatch set with a hue-optimized palette"). Two questions
Mike has not answered:

- **How many.** 8 gives 45° hue gaps and is comfortably distinguishable; 10 gives
  36° and is the conventional number; 12 sits at the 30° floor. Mean chroma is
  flat across the range, so size costs separation, not punch.
- **Whether to anchor a yellow in weasel's own set.** It costs ΔL 0.234 → 0.281
  and forces both a teal and a cyan into the set, and it is the only way to have
  a yellow at all.

**`--wzl-border-strong` fails WCAG 1.4.11's 3:1 in every combination** — dark
1.55 / 1.33 / 1.71, light 2.37 / 2.09 / 1.59. It is an assignment bug, not a ramp
bug: it sits two ramp steps from `surface` where it needs about four.
`--wzl-border-raised`, added in August for this, passes everywhere except
light-over-sunken, which its own docstring admits. **Worth asking whether these
are two tokens or one with two names.** Nothing is filed for this.

**A regenerated neutral ramp exists only in conversation and on the wall.**
weasel's shipping ramp is eyeballed, not computed: ΔL runs 0.043 → 0.154 (3.6×
spread) where an even walk is 0.089, and the dark end is crushed so 700/800/900
read as one field. A retuned proposal at 1.81× spread —
`#f5f6f7 #e0e1e4 #c6c8cb #a7a9ae #85888e #64676f #464a51 #2f3137 #1c1e22 #0c0e12`
— was measured and previewed but **deliberately not landed**: Mike said new
themes get generated ramps and weasel's own values stay pinned. Regenerating it
would move every surface color and is a visual-baseline event.
