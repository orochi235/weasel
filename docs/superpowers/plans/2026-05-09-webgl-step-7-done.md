...
- **`viewToMat3(view): Mat3` helper exported from `@weasel-js/gl`.** World-space layers wrap their `drawGL` output in `kind: 'group'` with this transform. Local `View`-shaped type (re-exported as `ViewLike`) avoids a runtime cross-package import.
...
- **Dev `vite.config.ts` aliased `@weasel-js/gl`** so dev pages can serve the browser through main `src/index.ts → pathLayer.ts → @weasel-js/gl` without bare-specifier failures. (See "Notable deviations.")
...
- **Dev vite alias is required, not optional.** The plan listed it under task 11 but didn't flag it as a blocker. In practice, the moment `src/features/paths/pathLayer.ts` started importing `@weasel-js/gl`, *every* dev page (including pre-existing `smoke.ts`, `colors.ts`, etc.) broke in the browser — the chain `weasel-gl/draw.ts → @weasel-js/core → src/index.ts → pathLayer.ts → @weasel-js/gl` hits an unresolved bare specifier. The vitest config already had this alias; the dev vite config didn't. Fixed by mirroring the alias from `vitest.config.ts` into `packages/gl/dev/vite.config.ts`. Filed as new convention §14.
...
- **Vitest config also gained `@weasel-js/gl` alias** as a side effect of the first cross-workspace value import (`viewToMat3`). Convention §4 anticipated this for the *first* such import — this is that moment. Documented but not novel.
...
- **Typecheck**: clean for `packages/gl` (modulo one pre-existing unrelated baseline error in `draw.ts(138,84)` that's not from this work).
- **Browser-verified**: layers smoke at `/packages/gl/dev/layers.html` shows the grid + green cell highlight + red and blue rects.
