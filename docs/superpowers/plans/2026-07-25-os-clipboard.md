...
## Smoke evidence (Task 6)

Chromium via Playwright MCP against `npm run dev:draw` (port 5175), clipboard-read/write permissions granted through the Playwright context. All numbered steps verified with recorded counts; no faked evidence.

1. **Copy writes real OS flavors.** Fresh starter scene (localStorage cleared; 2 nodes: `Welcome` rect + locked `Background`), starter rect selected via its Layers row (`sel: 1`), Copy toolbar button. `navigator.clipboard.read()` in page context returned one item with types `["web application/x-weasel-clipboard+json", "web image/svg+xml", "text/plain"]`. The custom flavor carried `{"weaselClipboard":1,"nodes":[{"kind":"leaf","id":"starter-1",...}]}`; `image/svg+xml` and `text/plain` both carried the selection-subset SVG (`viewBox="80 80 160 100"`, one `<path>`).
   - **Open question 2 answered:** Chromium's observed custom-MIME spelling is `web application/x-weasel-clipboard+json` (the `web `-prefix spelling) via the async clipboard API — and it round-trips only there. `ClipboardEvent.clipboardData.types` on Cmd+V was `["text/plain"]` — Chromium does not surface `web `-prefixed formats (nor the web-prefixed `image/svg+xml`) on the DOM paste event, so paste-event routing sees text/plain only.
2. **Same-tab Cmd+V:** node rows 2 → 3; exactly one new node, selection moved to it (`sel: 1`).
3. **Cross-reload paste (headline):** reload (in-memory ref gone), Cmd+V → rows 3 → 4. Per the finding in (1), the payload arrived as text/plain SVG and ingested through the `kit:svg` SVG-text fallback — structure and geometry preserved, labels not (SVG carries none). The `kit:weasel-json` handler path was separately proven in-browser by placing the weasel-JSON wire text on text/plain: one Cmd+V → one new node **with `label: "Welcome"` preserved** (full data fidelity).
4. **Group copy:** two nodes selected via Layers rows (click + shift-click, `sel: 2`), Cmd+G → container `n0-gf8os6`, `groups: 1`. Copy → custom flavor payload `[{container, parent:null}, {leaf, parent:container}, {leaf, parent:container}]` (parents-first subtree). Cmd+V → `groups: 1 → 2`, 3 new rows, new group selected — a GROUP pastes structurally.
5. **External SVG paste:** `<?xml …?><!DOCTYPE svg …>` preamble SVG (rect + circle) written via `writeText`, Cmd+V → rows 10 → 13, `groups: 2 → 3` (unpacked container + 2 leaves) through the SVG-text fallback. No regression from the higher-priority weasel handler.
6. **Draw→draw single-paste guard:** after every fresh weasel copy, one Cmd+V produced exactly one new node/group (steps 2–4 counts) — never two.

**Bug found and fixed during smoke (commit `17ec2310`):** draw wired the one-arg tree walker `reviveTypedArrays` directly as `ingestion.clipboard.reviver`, but the contract is a `JSON.parse`-style `(key, value)` reviver — called that way the walker returns each KEY string, collapsing the whole parse to `''`, so `kit:weasel-json` declined (dwarn: "weasel payload unusable") and dropped every weasel-JSON text payload. Fixed with `clipboardJsonReviver` in `apps/draw/src/persistence.ts` (one whole-tree revive pass at the root call) + red-first regression tests; re-verified in-browser (the label-preserving paste in step 3's second half ran through the fixed reviver).

**Known limitations (recorded, not regressions):** draw has no Cmd+C keybinding — copy is the toolbar button, exactly as before the migration (Cmd+V works via the DOM paste event); and because the paste event exposes only text/plain, draw→draw pastes ride the SVG flavor while the JSON-fidelity path serves consumers whose text/plain carries weasel JSON (the kit's default flavors).
