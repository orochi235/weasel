...
SVG is Swillustrator's *native* file format — Save and Open both use `.svg`. The persisted file is a valid SVG document that any browser, image viewer, or vector editor can render. Swillustrator-specific metadata that has no standard SVG equivalent (layers, paper-size enum, future parametric annotations) rides on a custom XML namespace (`xmlns:weasel="https://weasel.dev/svg-ext"`) — preserved on Swillustrator round-trip, gracefully ignored or stripped by third-party tools.
...
- **Multi-page documents v1.** Encoding is planned (`weasel:page` elements) but Swillustrator doesn't have multi-page in the UI yet. The spec reserves the encoding so it doesn't conflict; implementation is deferred.

## File format conventions

- **Extension:** `.svg`. No custom extension. The `xmlns:weasel` namespace declaration in the file is the marker that distinguishes a Swillustrator-authored SVG from a generic one.
- **MIME type:** `image/svg+xml`.
- **Namespace:** `xmlns:weasel="https://weasel.dev/svg-ext"`. Declared on the root `<svg>` element. The namespace URI does not need to resolve to an actual resource.
...
| **Paper size** | `viewBox="0 0 W H"` + `width`/`height` w/ unit suffix; named preset on `<svg weasel:paperSize="us-letter">` | ✗ | preset lost (named); dims preserved (✓) | T1 (one parser line + setDoc on open) |
| **Groups** | `<g>...</g>` with optional `weasel:group-id="..."` | ✗ broken both ways | ✓ | T2 (save walks `groupsRef`; open populates `groupsRef`; bridge stops flattening) |
| Layers | `<weasel:layers><weasel:layer id name visible/></weasel:layers>` at top; `weasel:layer-id="..."` on each shape | ✗ | lost (acceptable) | T3 (new feature; depends on layer model in Swillustrator) |
...
| Text style — line height | `weasel:line-height="1.4"` (no clean SVG attr) | ✗ | lost | T4 |
| Text style — fill (solid) | `fill="#rrggbb"` on `<text>` | partial | ✓ | T4 |
| Parametric origin (regular polygon, star) | `<weasel:parametric kind sides cx cy r/>` adjacent to its `<path>`, OR `weasel:kind="regular-polygon" weasel:sides="6"` attr on the path | ✗ (Swillustrator doesn't track this internally either) | lost | deferred — additive feature |
| Multi-page documents | `<weasel:page id label transform clip-rect>` at top; each shape carries `weasel:page-id` | ✗ (no UI) | first page only renders | deferred — additive feature |
...
<svg xmlns="http://www.w3.org/2000/svg"
     xmlns:weasel="https://weasel.dev/svg-ext"
     viewBox="0 0 816 1056"
     width="816"
     height="1056"
     weasel:paperSize="us-letter"
     weasel:units="px">
  <title>Untitled</title>
  <weasel:layers>
    <weasel:layer id="bg" name="Background" visible="true"/>
    <weasel:layer id="content" name="Content" visible="true"/>
  </weasel:layers>
  <g weasel:layer-id="bg">
    <path d="M 0 0 L 816 0 L 816 1056 L 0 1056 Z" fill="#ffffff"/>
  </g>
  <g weasel:layer-id="content" weasel:group-id="g1">
...
The `<weasel:layers>` registry is always at the top; layer membership is a per-shape attribute. Groups are real `<g>` elements with an optional id attribute. Anything Swillustrator-specific lives under the `weasel:` namespace; standard SVG content stays standard.
...
│    Obj ↔ SvgNode + weasel-namespace metadata          │
...
│    Knows about the weasel namespace                   │
└───────────────────────────────────────────────────────┘
```

### Where the weasel namespace lives

The `xmlns:weasel="..."` declaration is centralized in `@orochi235/weasel-svg`. weasel-svg already emits standard SVG; this spec extends it to also serialize known weasel-namespaced attributes/elements when present in the `SvgNode` tree. Specifically:

- New optional fields on `SvgNode` types (or a new `meta?: Record<string, unknown>` bag) carry the per-node weasel attrs.
- A new top-level option in `SerializeOptions` carries document-level extras (`paperSize`, `layers`, `pages`).
- `parseSvg` reads the weasel namespace symmetrically into the same shape.
...
- **T1: Paper-size round-trip.** Extend weasel-svg's parse to surface `viewBox`/`width`/`height`/`weasel:paperSize`/`weasel:units`. Extend serialize to write them. svgInterop calls `setDoc` on open and reads `doc` on save. Add round-trip tests with fixtures for `us-letter`, `a4`, and a custom size.
...
- **T3: Layers round-trip.** New encoding: `<weasel:layers>` element at top + `weasel:layer-id` per shape. Requires Swillustrator to actually have a layer model first (it has a `LayerList` UI but no persisted layer data per the audit — check this and either reuse the existing kit-level layer concept or design Swillustrator's flavor). Likely needs its own brainstorm before plan.
- **T4: Text style round-trip.** Map `TextStyle` fields to SVG `<text>` attributes (font-family, font-size, font-weight, font-style, fill). Line-height encodes as `weasel:line-height` since SVG has no clean attr for it. Round-trip tests for each field.
...
- **Multi-page documents.** Encoding is reserved (`<weasel:page>`) but no UI in Swillustrator. Defer until a real driver appears.
- **Parametric origin tracking.** Swillustrator doesn't preserve "this was a 5-pointed star" once the tool finishes. Adding that requires modeling parametric kinds in `Obj`, which is a separate Swillustrator feature, not a format question. Reserve `<weasel:parametric>` element for the future encoding.
...
- Saved files render correctly in Chrome / Safari / Firefox preview (no `weasel:` content visible; standard SVG only).
...
- weasel-svg's API surface stays clean: external consumers don't have to know about the `weasel:` namespace to use parseSvg / serializeSvg — the metadata is optional.
...
- **`<weasel:parametric>` elements** when Swillustrator starts tracking parametric origin for shapes.
- **`<weasel:page>` elements** when multi-page documents are designed.
...
- **A "Save flat SVG for sharing"** menu option that strips the `weasel:` namespace, producing a leaner third-party-friendly file. Useful for users who want a clean export.
...
- **`weasel:paperSize` enum drift:** picking a permissive set of paper-size names (`us-letter`, `us-legal`, `a4`, `a3`, `tabloid`, `custom`). If Swillustrator adds new presets, the enum grows additively — old files with unknown presets fall back to "custom" + the explicit width/height.
