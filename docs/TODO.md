...
- **General drag-gesture base subsuming `useDragRect` + `useMove` / `useResize` / `useRotate`.** Deferred from `docs/specs/2026-05-05-drag-insert-primitive-design.md`. `useDragRect` is shaped as a flat-file gesture base on the assumption it'll later be a subclass of a more general drag-gesture entry. Not designed yet; revisit when a third drag-rect gesture or a fourth pose-mutating gesture lands.
