...
- **Visual-regression coverage for the custom-shader demo.** Needs a deterministic-time hook on the demo (e.g. a query-string flag like `?frozenTime=0` that swaps the RAF loop for a fixed `u_time`). Once that exists, write a spec that captures one frame per panel.
