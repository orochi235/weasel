...
- **Pathfinder follow-ups (post-v1).** *Core five shipped 2026-05-11; see § "Path boolean operations — DONE 2026-05-11".* Remaining work, all deferred:
  ...
  - **Z-position of `useBooleans` result insert.** Spec calls for the result path to land at the topmost source's z-slot; v1 inserts via plain `createInsertOp` which goes wherever the adapter's `insertNode` defaults to (typically the top of the list). Wiring this through needs either an `at?: number` field on `createInsertOp` or a new `createInsertAtOp` — small surface change, deferred to avoid churning the op factory contract for one consumer.
