...
- **Widen `HotkeyTrigger` beyond modifier keys.** Today `Tool.hotkey` only accepts `'space' | 'alt' | 'ctrl' | 'meta' | 'shift'` and `useKeybindings`'s `HOTKEY_TRIGGER_MAP` only matches those physical keys. Nothing in the dispatcher actually requires a modifier — the slot is "engage while key is held," so a tool could plausibly be triggered by `'3'`, `'`'`, etc. Widen the type and the keymap, decide how this composes with regular `keybinding` switches (precedence, conflicts), and confirm key-repeat handling on bare letter/digit triggers.
