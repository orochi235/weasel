...
> **Status note (2026-06-15): the layout-reflow feature is currently disconnected from the runtime.** The drag-time layout pass lived in the `useMove` hook, which was deleted in `0d9c0759` ("phase 14e t4"). The replacement action path (`moveAction` + dispatcher + `usePreviewGhostLayer`) reparents on drop but runs **no** sibling reflow: `getLayout` on the synthesized adapter has **zero callers**, so the `LayoutStrategy` machinery (`getChildPositions`/`reflowFor`/`commitDrop`), the `layouts` option drilled `SceneCanvas → sceneToAdapter → adapter`, and the orphaned `MoveOverlay<TPose>` type (still exported from `index.ts`, never constructed) are all dead in the live path. The bullets below assume the feature is live and were written in the hook era — reconnecting reflow to `moveAction` is the prerequisite for most of them. Decide reconnect-vs-delete before picking any up.
...
- **(P2) Overlay rendering of reflowed siblings.** Two-step, because the data no longer flows (see status note above). (a) **Reconnect reflow to the action path** — `moveAction` consults `getLayout` during the drag, computes destination + source reflow, and exposes the resulting pose maps on its `OngoingHandle`. (b) **Render them** in `usePreviewGhostLayer` (this is the part the original bullet described, when it still named the dead `MoveOverlay` producer and `useSelectTool` overlay consumer). Likely drawn as ghosts. (a) is the prerequisite; without it there's nothing to draw.

- **(P3) Name-quality pass on the layout/overlay reflow surface.** The vocabulary is non-parallel and drifts from the kit's pose nomenclature. The orphaned `MoveOverlay<TPose>` pairs `hypotheticalChildPositions` (destination) with `sourceReflowPositions` (source) — same kind of thing (`Map<string, TPose>` of "where siblings would sit"), different head noun and qualifier. Both `…Positions` names hold *poses*, conflicting with the kit's `TPose`/"pose" usage everywhere else. On `LayoutStrategy`, `getChildPositions` (static layout) vs `reflowFor` (during-drag) don't telegraph that they're the same computation at two times. Want a parallel pair (e.g. `destReflow` / `sourceReflow`, both pose maps) and pose-aligned method names. Fold into whichever of the two steps above lands first, or do standalone if the feature is deleted rather than revived.
