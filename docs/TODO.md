...
- **(P3) Reconcile `BandEditor` with `Slider`.** `BandEditor` (bands: a contiguous tiling of an axis, seams draggable, each band carrying a payload) ships alongside `Slider` (a thumb list on an axis, `constraint: 'ordered'`, `onAddThumb`/`onRemoveThumb`, `renderTrack`). Under a contiguous tiling the two are the same control — N seams determine N+1 bands, so seams *are* an ordered thumb list — and they were kept separate deliberately: bridging them means teaching `Slider` about the region *between* thumbs (payload, hit-testing, selection), which is a wider change than the first consumer justified. Revisit when a second banding consumer appears, or when `Slider` next needs a non-linear axis. A third option arrived with `windease` 1.0 (2026-08-20): its `LayoutStrategy` is public API — `layout()` returns placements plus affordances, `reduce()` folds a gutter drag into strategy state — so a band control is a strategy you write rather than a control you build, and it brings widened gutter grab targets and `affects` for lock suppression with it — but not keyboard support: `LayoutEvent.kind` declares `'key'` and documents an arrow as arriving synthesized as a `'drag'`, yet nothing in the library synthesizes one, focuses a gutter, or emits ARIA (verified 2026-08-21 — Escape-cancels-a-drag is the only key it handles anywhere). It ships no band strategy of its own: the two built-ins are `gridStrategy` and `stripStrategy`, and strip is `LayoutStrategy<void>` whose gutters are single-child `resize-x` affordances writing pixel `placement.size`. Mapping domain values onto seams is still the consumer's. Note `Slider` is the former `RangePicker` (renamed in `9e934725`); `docs/specs/2026-05-09-range-picker-design.md` still uses the old name, and `RangeSlider`'s doc comment calling `Slider` "canvas-scrub" is stale from the same rename.

- **(P3) windease as a layout dependency — evaluated 2026-08-21, not adopted.** Two cases were checked against `windease` 1.0 (`~/src/windease`, `orochi235/windease` — a browser window manager: nodes with capabilities, pure `LayoutStrategy` functions, DnD, JSON snapshots). Both times the concepts line up and the integration doesn't pay yet. **A labkit sidebar of resizable tool palettes:** `stripStrategy` on `{ axis: 'y' }` brings gutters, per-child min/max, and pane sizes that round-trip through `serialize` for free — but there is no content-driven sizing, no collapse state (`hide` drops a node out of layout entirely, so collapse-to-header is a manual size write), and overflow squeezes panes past their minimum instead of scrolling. **labkit's `Workspace` tiling:** `gridStrategy` already auto-balances to `ceil(sqrt(n))`, so `packages/labkit/src/lab/gridDims.ts` and `WorkspaceGrid` would be deletions rather than ports — but labkit's zustand store owns workspace order, and a drop fights `reconcileChildOrder` with no controlled-order mode to resolve it, and grid ships no resize gutters, so "auto-balanced *and* resizable" isn't reachable. Nothing in weasel depends on windease today. Both gap lists are written up in `~/src/windease/TODO.md` under "Wishlist: docked tool palettes" and "Wishlist: hosting an app that already has a workspace store" — revisit when those ship, not before.
