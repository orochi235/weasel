...
- **Per-vertex coloring on paths.** *Renderer-side shipped, public surface deferred.* Step 5 added `PathDrawCommand.vertexColors?: number[]` (flat RGBA per-vertex, uploaded to a per-draw dynamic VBO; routed through a `pathFillVColor` shader). Consumers writing custom `RenderLayer.draw` can use it directly. What's missing on the public side: a Path-pose attribute (`Path.vertexColors`) that flows through `createPathLayer` automatically, plus `PoseDescriptor` interaction so resize/move don't drop the colors. Wire when a real consumer wants gradient maps / heatmaps / "rainbow stroke" effects through the kit's standard path layers rather than hand-rolling a custom renderer.
