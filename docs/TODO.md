...
- **(P1) Collapse the duplicated cascades the 2026-08-29 audit found.** The investigation is done — findings, sites and a verdict per pair are in `docs/superpowers/specs/2026-08-29-duplicated-cascade-audit.md`. What is left is the work. Read that spec's "Comments that assert a collapse which never happened" section first: seven docstrings tell a reader the collapse already landed, and each is false.
  ...
  One pair post-dates the audit: the removal closure is now computed twice — `removalClosure` in `core/scene/scene.ts` walks down from the roots, `coveredByEmitted` in `interactions/actions/defaults/delete.ts` walks up from each candidate, because no public surface exposes the reverse `dependsOn` index. It has already drifted once, in the arc that created it: adding the dependents relation to the scene without adding it to `delete.ts` made the built-in Delete key throw mid-batch on a selection holding a node and its edge.
