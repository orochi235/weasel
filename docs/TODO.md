...
- **Remove orphaned `defaultLayers` helper module.** `src/canvas/layers.ts` (`defaultLayers` + `DefaultLayersOpts` + `DefaultLayersScene`/`DefaultLayersSelection`) was the helper that folded `moveOverlay`/`resizeOverlay` into the scene draw. After the inline-gesture strip (Task 8 of `docs/plans/2026-05-05-canvas-inline-gesture-cleanup.md`), nothing in `Canvas.tsx`, demos, or apps consumes it — only its own test file references it. Delete the module + test, drop the four barrel exports. Held back from the cleanup commit out of conservatism. Surfaced during Task 9 barrel audit.
