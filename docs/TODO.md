...
- **NestedGroupsDemo: collapse custom moveTool + custom scene/ghost layers into `useSelectTool` once the standard scene slot supports world-pose mode.** *Partial migration shipped (2026-05-05):* hit resolution moved into `nestedGroupHitTester(adapter, …)` exported from the kit, and the demo's pointer.onDown calls `hitter.pickBest(...)` (alt-aware) while `<Canvas hitBody>` uses `hitter.pickOutermost`. The custom inline moveTool still wraps the cascade-aware `useMove` directly because the demo's `sceneLayer`/`ghostLayer` read `move.overlay.poses` / `hideIds` to render in world coords (poses are LOCAL on disk). Migrating fully to `useSelectTool({ pickBest })` requires (a) a world-pose mode on the standard scene slot — `drawOne(ctx, obj, worldPose)` instead of local — so the kit's bundled scene+ghost render replaces the custom layers, and (b) wiring `hitTestArea`/`applyOps` on the demo adapter so area-select can run.
