...
- **(P3) Should scene/op composition APIs accept a `d` string directly?** `pathFromD(d)` shipped 2026-06-27 (`src/features/paths/pathFromD.ts`; the `d`-grammar parser moved out of `@weasel-js/svg` into core, which now owns it and the svg document parser imports it back). Per the **terse path language** stance in `docs/conventions.md`, SVG `d` is the terse composition surface — power without a new external standard, builders still co-equal. Remaining open question: whether insert/scene/op composition APIs should accept the raw `d` string inline (`insert({ d: "M0 0 …" })`) or keep requiring an explicit `pathFromD(d)` call at the call site. Decide when a composition API would actually carry it.
