...
- **Groupable objects.** First-class group node: select-as-one, move-as-one, transform children relative to group origin. Universal across diagramming and illustration tools. *Status:*
  ...
  - Open follow-ups: cut over snap behaviors (e.g. `gridSnapStrategy`) so they snap in world space when a parent translation is in play; selection overlay needs a convenience composer so consumers don't hand-write `composeWorldPose` calls per id; `useMoveInteraction` could grow auto-cascade so dragging a group with a structurally-grouped child shifts the child's world without the consumer rebasing.
