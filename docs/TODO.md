...
- **Reactive cursor updates during in-flight gesture.** Function-form `cursor` is now wired (Canvas's `resolveToolsCursor` calls it with the live base ctx and the dispatcher's current scratch via `getActiveScratch()`). But cursor only re-resolves when `<Canvas>` re-renders, so scratch transitions inside the dispatcher (idle → drag, mid-drag mutations) don't update the DOM cursor on their own. `tools.modifierEngaged` transitions DO trigger re-render, so modifier-driven cursors (e.g. hand-while-space) update correctly. To get reactive grab→grabbing during a drag, add a `gestureTick` state on `useTools` that bumps on gesture start/end (and exposes via `ToolsApi`); Canvas reads it as a render dep. Defer until a real consumer wants it.
...
- **Drop the remaining inline `useMove`/`useResize`/`useRotate` + scene-layer overlay fold-in from `Canvas.tsx`.** Task 11 of the tool-overlay-channel plan stripped the `tool=` shorthand and the now-orphaned `useInsert`/`useAreaSelect` hooks, but kept the inline `useMove`/`useResize`/`useRotate` controllers (and the `buildSceneLayer` fold-in of `moveOverlay`/`resizeOverlay`/`rotateOverlay`) because legacy demos (`MoveDemo`, `ActionsDemo`, `GroupsDemo`, `CloneDemo`, `BezierEditDemo`, `PathPoseDemo`, plus several Canvas tests) still drive body/handle drags through the adapter prop without a `tools={}` migration. Finish the cleanup by either (a) migrating those demos + tests to a `useSelectTool`-style `tools` setup and removing the inline gesture controllers entirely, or (b) extracting an explicit "legacy mode" Canvas variant so the main `<Canvas>` only hosts the Tool dispatcher path. Once done, also drop `effectivePoseOf` / `baseBoundsOf` closures and the move/resize/rotate fold-in inside `buildSceneLayer` — committed poses only; the active Tool's overlay paints ghosts.
...
- **SceneCanvas's default tool no-ops marquee area-select.** SceneCanvas's synthesized adapter stubs `hitTestArea: () => []` and `applyOps: () => {}` to satisfy `useSelectTool`'s `AreaSelectAdapter` constraint, so empty-space drag over a Scene doesn't select anything via the default tool. Consumers wanting marquee-select-over-Scene must currently supply their own `tools={...}`. Surfaced during Task 7.5 SceneCanvas migration. Fix when a real consumer asks: implement default `hitTestArea` from `scene.renderOrder()` + per-pose AABB intersect, plus `applyOps` that routes setPose ops through the synthesized adapter.
- **Tools dispatcher hard-codes pointer modifiers as `false`.** `Canvas`'s `getCtx` for the tools path passes `modifiers: { alt:false, shift:false, meta:false, ctrl:false }` instead of reading from the pointer event. Shift-click multi-extend, alt-clone, etc. cannot fire through the tool path until this reads `e.shiftKey`/`e.altKey`/`e.metaKey`/`e.ctrlKey`. Surfaced when porting Canvas.test.tsx multi-selection tests; the shift-extend assertion was dropped from the ported test.
- **Tools dispatcher bypasses `<Canvas>`'s `clientToWorld` prop.** Tools-path pointer events go through their own coord conversion that ignores the `clientToWorld` override (used by BezierEditDemo/PathPoseDemo for zoom). Inline path respected it. Either thread `clientToWorld` into the dispatcher or document the limitation. Surfaced during Task 7 (Canvas.test.tsx port).
- **Tool `debug` sink doesn't reach Canvas's debug overlay layer.** `useSelectTool({ debug })` accepts a `DebugSink` but Canvas builds its own sink internally from its `debug: DebugConfig` prop for the visible overlay. So tool-internal hitbox/handle records are sunk into a parallel sink with no renderer, and corner-handle/rotation-handle hitbox visualization that worked pre-migration is silently dropped on tool-path consumers. Surfaced during PathPoseDemo migration. Fix: either let Canvas accept an externally-built sink, or have the tool accept a `DebugConfig` and build its own sink shared with Canvas's overlay.
- **`useTextTool` still captures `ctx.applyBatch` into a ref on each handler entry.** The shim is now small (one option to `useInsert`, no fake adapter methods) and explicitly tested at the dispatch-routing seam, but it remains asymmetric with `useInsertTool`'s consumer-owned adapter. Revisit if a third drag-insert tool would benefit from a unified ergonomics story (e.g. accept either an adapter *or* an inline factory).
