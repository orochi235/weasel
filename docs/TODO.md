...
- [x] **Visual-regression specs for the gradient / vertex-colors / color-matrix demos.** *Specs added 0.3.0.* Three new spec files under `tests/visual/`. Baselines are `.gitignore`d for the whole rig (per `tests/visual/baselines/.gitignore`: deferred until the CI soak procedure can capture them on ubuntu-22.04 — locally-captured PNGs won't match). All three pass locally against fresh baselines. Custom-shader demo is excluded because each panel animates via `requestAnimationFrame` — pixel-perfect comparison is unstable without a "freeze time" hook.
- **Visual-regression baselines: run the CI soak.** Pending blocker for the entire rig (not just the new demos). The Step 9 plan called for a soak run on ubuntu-22.04 to capture canonical baselines, then committing them and removing `tests/visual/baselines/.gitignore`. Until that happens, every spec runs in "first-run" mode (writes a baseline, asserts nothing) — the rig can't catch regressions yet.
...
- **Modernize the existing visual-regression specs.** Specs still use the dual-test shape (`?backend=2d` baseline + `?backend=gl` match) from before the WebGL transition. The URL param is now a no-op so both runs hit the same canvas; collapse each spec to a single check and drop the `-2d` baseline suffix. Bundle this with the soak run so baselines and specs flip together.
