...
"@weasel-js/hud": major
"@weasel-js/modes": minor
---

A registry entry declares what it contributes and when it is eligible.
...
holds several: the hand tool is palette-selectable *and* space-held. `focus`,
...
*id*, which `engagedIds` reads, so the declaration registers the binding while
