...
const DEMO_META: DemoMeta[] = [
  ...
  },
  {
    ...
    category: 'Viewport',
    description: 'SceneCanvas viewport prop wires inertia pan, pinch zoom, and keyboard zoom in one place. Inertia uses a friction-decayed velocity loop after drag release; boundary clamping can stop or bounce the pan at configurable limits. A touch-screen pinch reaches the viewport.pinchZoom action through the dispatcher\'s two-finger multitouch stream; a trackpad pinch emits no touch pointers at all, so the browser sends ctrl+wheel and viewport.zoom claims that instead.',
    hint: 'Drag fast and release to coast · ⌘+= / ⌘+- / ⌘+0 to zoom · pinch or ⌘+wheel to zoom · toggle boundary to see stop vs bounce.',
