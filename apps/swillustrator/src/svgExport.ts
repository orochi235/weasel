/**
 * Kit-native SVG export for the swill scene.
 *
...
import { docToSerializeOptions, type SwillPaperSize } from './svgInterop';
...
function pathAtPose(path: Path, pose: SwillPose): Path {
...
function leafToSvgNode(data: SwillData, pose: SwillPose): SvgNode | null {
  if (data.text != null) {
...
export interface SceneToSvgOptions {
  ...
  filename: string;
  /** Paper size key. Round-trips via `swill:paperSize`. */
