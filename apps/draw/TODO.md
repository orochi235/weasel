# WeaselDraw v0 — deferred work

The post-purge rebuild (chore/wd-rebuild) shipped a minimum-viable editor.
...
- **Groups.** SceneCanvas's standard actions register `group` / `ungroup`,
  but WeaselDraw's `groupMembership.ts` helpers aren't connected; we
