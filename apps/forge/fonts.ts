...
/** Storybook's font toolbar, as forge globals: a family per weasel font slot, then weight, width and italic. The
 *  slots past the first are set per trial rather than from the toolbar. */
export const FONT_GLOBALS: GlobalDeclarations = {
  fontFamily: { label: 'Font', default: 'oswald', options: options(FAMILY_OPTIONS) },
  ...Object.fromEntries(
    // A trial's Globals row shows a select's value alone, so each option names its slot.
